...
exports.deleteUser = exports.updateUser = exports.createUser = exports.getUserByGoogleId = exports.getUserById = exports.getAllUsers = void 0;
const mongodb_1 = require("mongodb");
const connection_1 = __importDefault(require("../db/connection"));
const utilities_1 = require("../utilities");
const getAllUsers = async (req, res) => {
    try {
        const result = await connection_1.default.getDb().db('RandR').collection('User').find().toArray();
...
const getUserById = async (req, res) => {
    try {
        ...
        const result = await connection_1.default.getDb().db('RandR').collection('User').findOne({ _id: id });
        res.status(200).json(result);
    }
    catch (err) {
        if (err instanceof Error) {
...
const getUserByGoogleId = async (req, res) => {
    try {
        ...
        const result = await connection_1.default.getDb().db('RandR').collection('User').findOne({ googleId: id });
        res.status(200).json(result);
    }
    catch (err) {
        if (err instanceof Error) {
...
const createUser = async (req, res) => {
    try {
        const { profile, experienceLevel = 1, primaryGoal = "general_fitness", secondaryGoals = [], availableEquipment = [], workoutDurationMinutes = 30, contraindications = [], activeInjuries = [] } = req.body;
...
const updateUser = async (req, res) => {
    try {
        ...
        const flattenedUpdates = (0, utilities_1.flattenObject)(req.body);
        const updates = {};
        ...
        const db = connection_1.default.getDb().db("RandR");
        const result = await db.collection("User").updateOne({ _id: new mongodb_1.ObjectId(userId) }, { $set: updates });
...
const deleteUser = async (req, res) => {
    const id = new mongodb_1.ObjectId(req.params.id);
    const del = await connection_1.default.getDb().db('RandR').collection('User').deleteOne({ _id: id });
